...
class ProcessMakerWebDav extends HTTP_WebDAV_Server
{
    ...
    {
        //$this->base = '/';
        $this->uriBase = '/sys' . config("sys_sys") . '/' . SYS_LANG . '/' . SYS_SKIN . '/services/webdav/';
    ...
    public function PROPFIND(&$options, &$files)
    {
        ...
        $pathClasses = PATH_DB . PATH_SEP . config("sys_sys") . PATH_SEP . 'classes' . PATH_SEP;
        ...
        $pathProcesses = PATH_DB . config("sys_sys") . PATH_SEP;
    ...
    public function GET(&$options)
    {
        ...
        $pathClasses = PATH_DB . config("sys_sys") . PATH_SEP . 'classes' . PATH_SEP;
        if (count($paths) > 0 && $paths[0] == 'classes' && is_dir($pathClasses)) {
