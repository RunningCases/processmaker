...
class Processes
{
        ...
        try {
            ...
            foreach ($arrayData as $value) {
                ...
                try {
                    ...
                    Bootstrap::registerMonolog('DataError', 400, $e->getMessage(), $record, config("sys_sys"), 'processmaker.log');
                }
            }
        } catch (Exception $e) {
            throw $e;
    ...
    public function saveSerializedProcess($oData)
    {
        ...
        $pathLength = strlen(PATH_DATA . "sites" . PATH_SEP . config("sys_sys") . PATH_SEP . "files" . PATH_SEP . "output" . PATH_SEP);
        ...
        $MAILS_ROOT_PATH = PATH_DATA . 'sites' . PATH_SEP . config("sys_sys") . PATH_SEP . 'mailTemplates' . PATH_SEP . $data->process['PRO_UID'];
        ...
        // for public files
        $PUBLIC_ROOT_PATH = PATH_DATA . 'sites' . PATH_SEP . config("sys_sys") . PATH_SEP . 'public' . PATH_SEP . $data->process['PRO_UID'];
...
//        $this->createTaskExtraPropertiesRows( isset( $oData->taskExtraProperties ) ? $oData->taskExtraProperties : array () );
    ...
    public function getDisabledCode($processUid = null, $workspaceName = null)
    {
        try {
            ...
            //Set variables
            $cs = new CodeScanner((!is_null($workspaceName))? $workspaceName : config("sys_sys"));
