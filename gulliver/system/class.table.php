...
class Table
{
    ...
    public function RenderTitle ($pa, $intPos = 1, $strClass = "tblHeader")
    {
        ...
        if ($this->_ordered == true && $order) {
            ...
            $res .= (ENABLE_ENCRYPT == 'yes' ? str_replace( G::encrypt( 'sys' . config("sys_sys"), URL_KEY ), config("sys_sys"), G::encryptUrl( urldecode( SYS_CURRENT_URI ), URL_KEY ) ) : SYS_CURRENT_URI) . "?order=" . $this->Columns[$intPos]['Name'] . "&page=" . $pa . "&label=true";
